...
def _set_test_cache(state_id, _):
    ...
    """
    LAST_PARSED.set_last_parsed_manifest(state_id, TEST_PROJECT_PATH,
                                         TEST_ROOT_PATH, TEST_MANIFEST,
                                         TEST_MANIFEST_SIZE, TEST_CONFIG)
...
def _get_test_cache(state_id: str, project_path: str = TEST_PROJECT_PATH):
    ...
    `project_path`.
    """
    return CachedManifest(state_id, project_path, TEST_ROOT_PATH,
                          TEST_MANIFEST, TEST_MANIFEST_SIZE, TEST_CONFIG,
                          TEST_SQL_PARSER)
...
class TestCachedManifest(TestCase):
    ...
    def test_set_last_parsed_manifest(self, mock_get_sql_parser):
        LAST_PARSED.set_last_parsed_manifest(TEST_STATE_ID, TEST_PROJECT_PATH,
                                             TEST_ROOT_PATH, TEST_MANIFEST,
                                             TEST_MANIFEST_SIZE, TEST_CONFIG)
    ...
    def test_lookup_state_cache_matched(self):
        ...
        self.assertEqual(LAST_PARSED.lookup(TEST_STATE_ID),
                         _get_test_cache(TEST_STATE_ID))
...
class TestStateController(TestCase):
    ...
    def test_from_cached(self):
        self._assert_state_controller_equals(StateController.from_cached(
            _get_test_cache(TEST_STATE_ID)),
            StateController(
            TEST_STATE_ID,
            TEST_PROJECT_PATH,
    ...
    @mock.patch("dbt_server.services.filesystem_service.get_root_path",
                return_value=TEST_ROOT_PATH)
    @mock.patch("dbt_server.services.dbt_service.parse_to_manifest",
                return_value=TEST_MANIFEST)
    @mock.patch("dbt_server.services.dbt_service.create_dbt_config",
                return_value=TEST_CONFIG)
    @mock.patch("dbt_server.services.dbt_service.get_sql_parser",
                return_value=TEST_SQL_PARSER)
    def test_parse_from_source(self, mock_get_sql_parser,
                               mock_create_dbt_config, mock_parse_to_manifest,
                               mock_get_root_path):
        parse_args = ParseArgs(state_id=TEST_STATE_ID,
        ...
        mock_get_root_path.assert_called_once_with(TEST_STATE_ID,
                                                   TEST_PROJECT_PATH)
        mock_create_dbt_config.assert_called_once_with(TEST_ROOT_PATH,
                                                       parse_args)
        mock_get_sql_parser.assert_called_once_with(TEST_CONFIG, TEST_MANIFEST)
        mock_parse_to_manifest.assert_called_once_with(TEST_ROOT_PATH,
                                                       parse_args)

    def test_load_state_non_state_cached(self):
        args = ParseArgs(state_id=None)
        _set_test_cache(None)
        self._assert_state_controller_equals(StateController.load_state(args),
                                             StateController(
            None,
            TEST_PROJECT_PATH,
            TEST_ROOT_PATH,
    ...
    @mock.patch("dbt_server.services.filesystem_service.get_latest_state_id",
                return_value=None)
    @mock.patch("dbt_server.services.filesystem_service.get_latest_project_path",
                return_value=None)
    def test_load_state_no_cache_state_id_missing(self,
    ...
    @mock.patch("dbt_server.services.filesystem_service.get_root_path",
                return_value=TEST_ROOT_PATH)
    @mock.patch("dbt_server.services.dbt_service.deserialize_manifest",
                return_value=TEST_MANIFEST)
    @mock.patch("dbt_server.services.filesystem_service.get_size",
                return_value=TEST_MANIFEST_SIZE)
    @mock.patch("dbt_server.services.dbt_service.create_dbt_config",
                return_value=TEST_CONFIG)
    @mock.patch("dbt_server.services.dbt_service.get_sql_parser",
                return_value=TEST_SQL_PARSER)
    def test_load_state_no_cache_has_project_path(self,
                                                  mock_get_sql_parser,
        ...
        mock_get_root_path.assert_called_once_with(None,
                                                   TEST_PROJECT_PATH)
        mock_get_latest_state_id.assert_called_once_with(None)
        mock_get_latest_project_path.assert_called_once_with()
        mock_deserialize_manifest.assert_called_once_with(
            TEST_MANIFEST_FILE)
    ...
    @mock.patch("dbt_server.services.dbt_service.get_sql_parser",
                return_value=TEST_SQL_PARSER)
    def test_load_state_no_cache_has_state_id(self,
                                              mock_get_sql_parser,
                                              mock_create_dbt_config,
                                              mock_get_size,
                                              mock_deserialize_manifest,
    ...
    @mock.patch("dbt_server.services.filesystem_service.get_partial_parse_path",
                return_value=TEST_PARTIAL_PARSE_FILE)
    @mock.patch("dbt_server.services.dbt_service.serialize_manifest",
                return_value=TEST_MANIFEST)
    @mock.patch("dbt_server.services.filesystem_service.get_size",
                return_value=TEST_MANIFEST_SIZE)
    def test_serialize_manifest(self, mock_get_size, mock_serialize_manifest,
                                mock_get_partial_parse_path):
    ...
    def test_update_cache_state_id(self, mock_get_sql_parser,
        ...
        state_controller = StateController(
            TEST_STATE_ID, None, TEST_ROOT_PATH, TEST_MANIFEST,
    ...
    @mock.patch("dbt_server.services.dbt_service.compile_sql",
                return_value=TEST_VALUE)
    def test_compile_query(self, mock_compile_sql):
        state_controller = StateController(
            None, None, TEST_ROOT_PATH, TEST_MANIFEST,
            TEST_CONFIG, TEST_SQL_PARSER, None, None)
        self.assertEqual(state_controller.compile_query(TEST_QUERY), TEST_VALUE)
        mock_compile_sql.assert_called_once_with(TEST_MANIFEST, TEST_CONFIG,
                                                 TEST_SQL_PARSER, TEST_QUERY
                                                 )

    @mock.patch("dbt_server.services.dbt_service.execute_sql",
                return_value=TEST_VALUE)
    def test_execute_query(self, mock_execute_sql):
        ...
        mock_execute_sql.assert_called_once_with(TEST_MANIFEST, TEST_ROOT_PATH,
                                                 TEST_QUERY
                                                 )
    ...
    def test_execute_async_command(self, mock_execute_async_command):
        state_controller = StateController(
            TEST_STATE_ID, None, TEST_ROOT_PATH, TEST_MANIFEST,
            None, None, None, None)
        state_controller.execute_async_command(TEST_TASK_ID, TEST_COMMAND,
                                               TEST_DB, TEST_CALLBACK_URL)
        mock_execute_async_command.assert_called_once_with(TEST_COMMAND,
                                                           TEST_TASK_ID,
                                                           TEST_ROOT_PATH,
                                                           TEST_MANIFEST,
                                                           TEST_DB,
    ...
    @mock.patch("dbt_server.services.dbt_service.execute_sync_command",
                return_value=TEST_VALUE)
    def test_execute_sync_command(self, mock_execute_sync_command):
        state_controller = StateController(
            None, None, TEST_ROOT_PATH, TEST_MANIFEST,
            None, None, None, None)
        self.assertEqual(state_controller.execute_sync_command(
            TEST_COMMAND), TEST_VALUE)
        mock_execute_sync_command.assert_called_once_with(TEST_COMMAND,
                                                          TEST_ROOT_PATH,
                                                          TEST_MANIFEST)
