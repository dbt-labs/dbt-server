...
from dbt_server.database import get_redis_client

from dbt_worker.app import app as celery_app
from dbt_worker.tasks_temp import invoke
...
class DbtGetCommandArgs(BaseModel):
    task_id: Optional[str]


class DbtPutCommandArgs(BaseModel):
    task_id: Optional[str]
...
# sementic layer: push parse compile(we want them to use sync)
...
@app.post("/async/dbt_test")
async def dbt_entry_async_test_post(
    args: DbtCommandArgs
):
    task = invoke.delay(args.command, args.project_dir)
    task_id = task.id
    # Testing purpose. Syncs with worker.
    log_dir_path = os.path.join(args.project_dir, "logs", task_id)
...
@app.get("/async/dbt_test")
async def dbt_entry_async_test_get(
    args: DbtGetCommandArgs,
    redis_client: StrictRedis = Depends(get_redis_client)
):
    task = redis_client.get(f"celery-task-meta-{args.task_id}")
    if task is None:
        return JSONResponse(
            status_code=200,
            content={
...
@app.put("/async/dbt_test")
async def dbt_entry_async_test_cancel(args: DbtPutCommandArgs):
    res = AbortableAsyncResult(args.task_id, app=celery_app)
    if res.state in READY_STATES:
        return JSONResponse(
            status_code=200,
            content={
                "status": f"Task can't be cancelled, status is {res.state}"
            },
        )
    res.abort()
